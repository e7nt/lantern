...
HELIX_PATCHES=(
	"$SPIKE_DIR/patches/0001-add-lantern-range-navigation.patch"
	"$SPIKE_DIR/patches/0002-add-picker-mouse-interaction.patch"
)
HELIX_PATCHED_FILES=(
...
if git -C "$HELIX_DIR" diff --quiet; then
	...
	echo "Helix contains changes other than the exact Lantern spike patch." >&2
	echo "Use a clean checkout at $HELIX_REVISION before preparing the spike." >&2
	exit 1
fi

cargo build --release --locked --manifest-path "$HELIX_DIR/Cargo.toml"
cargo build --release --locked --manifest-path "$SPIKE_DIR/runtime/Cargo.toml"
...
printf 'Prepared Helix %s, Lazygit %s, and the Lantern spike runtime.\n' \
